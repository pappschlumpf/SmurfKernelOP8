...
static unsigned int boost_little_off(struct sugov_policy *b_policy) {
...
static unsigned int boost_little(struct sugov_policy *b_policy, unsigned long load) {
...
static unsigned int boost_big_off(struct sugov_policy *b_policy) {
...
static unsigned int boost_big(struct sugov_policy *b_policy, unsigned long load) {
 ...
 */
unsigned long smurfutil_freq_util(int cpu, unsigned long util,
				  unsigned long max, enum smurfutil_type type)
{
	unsigned long dl_util, irq;
	struct rq *rq = cpu_rq(cpu);

	if (sched_feat(SUGOV_RT_MAX_FREQ) && type == FREQUENCY_UTIL_SMU &&
...
static unsigned long sugov_get_util(struct sugov_cpu *sg_cpu)
{
	...
	return smurfutil_freq_util(sg_cpu->cpu, util, max, FREQUENCY_UTIL_SMU);
}
#endif
